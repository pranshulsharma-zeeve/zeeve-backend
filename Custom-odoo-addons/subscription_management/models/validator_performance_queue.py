...
class ValidatorPerformanceQueue(models.Model):
    _name = 'validator.performance.queue'
    _description = 'Validator Performance Queue'
    ...
    def populate_performance_queue(self):
        """
        Populate the queue with validators from protocols that support performance snapshots.
        ...
        """
        _logger.info("Starting queue population for validator performance")
        ...
        _logger.info(f"Evaluating {len(nodes)} validator nodes for performance queue")
        ...
        _logger.info(f"Extracted {len(validators)} validators ready for performance snapshots")
