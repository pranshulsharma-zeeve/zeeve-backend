...
class CancelWizard(models.TransientModel):
    ...
    def get_cancel_subscription(self):
        ...
        if subscription_obj.get_cancel_sub():
            reason_text = self.reason_id.name + (self.comment if self.comment else "")
            subscription_obj.reason = reason_text
            subscription_obj._send_cancellation_notifications(cancellation_reason=reason_text)
        return True
