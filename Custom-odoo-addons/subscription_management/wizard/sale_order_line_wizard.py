...
class AddProduct(models.TransientModel):
    ...
    product_id =  fields.Many2one('product.product',string='Product', domain=[('sale_ok', '=', True),('activate_subscription','=',True), ('subscription_plan_id.active','=',True)], change_default=True, ondelete='restrict', required=True)
    ...
    def create_sale_order_line(self):
        ...
        if sale_order.pricelist_id and sale_order.partner_id:
            ...
            if not override:
                line_id.price_unit = self.env['account.tax']._fix_tax_included_price_company(line_id._get_display_price(), product.
